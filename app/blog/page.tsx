...
export default function BlogPage() {
  return <main className="inner-page">
    ...
    <section className="article-grid shell">{articles.map(article => <a href={`/blog/${article.slug}`} className="article-card" key={article.slug}><span>{article.category}</span><h2>{article.title}</h2><p>{article.excerpt}</p><em>{article.date} <ArrowUpRight /></em></a>)}</section>
    <SiteFooter />
  </main>;
}
