...
export default function PricingPage(){
 ...
 return <main className="inner-page pricing-page">
  ...
  <section className="pricing-models shell">{models.map(m=><article key={m[0]}><h2>{m[0]}</h2><p>{m[1]}</p><ul>{m.slice(2).map(x=><li key={x}><CheckCircle2 size={16}/>{x}</li>)}</ul></article>)}</section>
