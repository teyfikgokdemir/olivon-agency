...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      type: "website",
      siteName: "Olivon",
      locale: marketContent[locale].ogLocale,
      title: page.metaTitle,
      description: page.metaDescription,
      url: `${SITE_URL}${canonical}`,
      images: [{ url: "/images/olivon-og.webp", width: 1200, height: 630, alt: "Olivon digital growth studio" }],
    },
...
export default async function IntlLanding({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const webPage = {
    ...
    name: page.metaTitle,
    description: page.metaDescription,
    inLanguage: marketContent[locale].htmlLang,
    isPartOf: { "@id": `${SITE_URL}/#website` },
    about: { "@id": `${SITE_URL}/#organization` },
    breadcrumb: { "@id": `${SITE_URL}${canonical}#breadcrumb` },
  };
  const service = ["services","ecommerce","search","web","security"].includes(key) ? {
    "@context": "https://schema.org",
    "@type": "Service",
    ...
    areaServed: locale === "de" ? ["DE","AT","CH"] : locale === "fr" ? ["FR","BE","CH"] : ["GB","EU","TR"],
  } : null;
    ...
    <main data-locale={locale}>
      ...
      <section className="page-hero shell">
        ...
        <div className="hero-actions">
          <a className="button primary" href={localizedPath(locale,"contact")}>{marketContent[locale].primary} <ArrowUpRight size={18} /></a>
          {key !== "services" && <a className="button ghost" href={localizedPath(locale,"services")}>{marketContent[locale].nav.services}</a>}
        </div>
      </section>

      {page.sections.length > 0 && (
        <section className="services shell intl-detail-grid" aria-label={page.title}>
          <div className="service-list">
            {page.sections.map(section => (
              <article className="service-row" key={section.title}>
                <span><h2>{section.title}</h2><p>{section.text}</p>{section.bullets && <ul>{section.bullets.map(item => <li key={item}><Check size={15}/>{item}</li>)}</ul>}</span>
              </article>
            ))}
          </div>
        </section>
      ...
      {page.faq?.length ? (
        <section className="faq shell" aria-labelledby="intl-faq-title">
          <div className="section-head"><div><p className="section-index">FAQ</p><h2 id="intl-faq-title">{key === "faq" ? page.title : "FAQ"}</h2></div></div>
          <div className="faq-list">{page.faq.map(([question, answer]) => <details key={question}><summary>{question}</summary><p>{answer}</p></details>)}</div>
        </section>
      ) : null}

      <section className="manifesto shell localized-contact">
        <p className="section-index">OLIVON</p>
        <h2>{marketContent[locale].ctaTitle}</h2>
        <p>{marketContent[locale].ctaText}</p>
        <div className="hero-actions"><a className="button primary" href={localizedPath(locale,"contact")}>{marketContent[locale].primary} <ArrowUpRight size={18}/></a></div>
      </section>
      <SiteFooter />
    </main>
  );
