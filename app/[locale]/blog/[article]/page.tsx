...
import { intlArticleBySlug, intlArticles } from "@/lib/intl-articles";
...
export function generateStaticParams(){return locales.flatMap(locale=>intlArticles[locale].map(article=>({locale,article:article.slug})));}
...
export async function generateMetadata({params}:{params:Promise<{locale:string;article:string}>}):Promise<Metadata>{
  ...
  const article=intlArticleBySlug(locale,slug);
  if(!article) return {};
  const canonical=`/${locale}/blog/${article.slug}`;
  return {title:`${article.title} | Olivon`,description:article.excerpt,alternates:{canonical},openGraph:{type:"article",siteName:"Olivon",locale:marketContent[locale].ogLocale,title:article.title,description:article.excerpt,url:`${SITE_URL}${canonical}`,publishedTime:article.dateISO},other:{"content-language":marketContent[locale].htmlLang}};
}
...
export default async function LocalizedArticle({params}:{params:Promise<{locale:string;article:string}>}){
  ...
  return <main data-locale={locale} lang={marketContent[locale].htmlLang} className="inner-page">
    ...
    <section className="manifesto shell localized-contact"><p className="section-index">OLIVON</p><h2>{cta.title}</h2><p>{cta.text}</p><div className="hero-actions"><a className="button primary" href={`/${locale}/${locale==='de'?'kontakt':locale==='fr'?'contact':'contact'}`}>{cta.button} <ArrowUpRight size={18}/></a></div></section>
    <SiteFooter/>
  </main>;
}
