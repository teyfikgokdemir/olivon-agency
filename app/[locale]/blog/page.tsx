...
const labels: Record<Locale, { kicker:string; title:string; intro:string; read:string; metaTitle:string; metaDescription:string }> = {
  ...
  de:{kicker:"INSIGHTS",title:"Klare Notizen für bessere digitale Entscheidungen.",intro:"E-Commerce, Migration, internationales SEO, KI-Sichtbarkeit und Plattformfragen für Teams, die im DACH-Markt sauber entscheiden wollen.",read:"Artikel lesen",metaTitle:"Insights | E-Commerce, SEO & KI für DACH | Olivon",metaDescription:"Praxisnahe Beiträge zu E-Commerce, Migration, SEO, GEO, AEO, AIO und Plattformentscheidungen für den DACH-Markt."},
  fr:{kicker:"INSIGHTS",title:"Des repères clairs pour de meilleures décisions digitales.",intro:"E-commerce international, SEO, visibilité IA, systèmes web et choix de plateforme pour les équipes qui veulent avancer avec méthode.",read:"Lire l’article",metaTitle:"Insights | E-commerce, SEO & IA | Olivon",metaDescription:"Guides pratiques sur l’e-commerce international, le SEO, GEO, AEO, AIO, le web et les plateformes pour les marchés francophones."},
};
...
export default async function LocalizedBlog({params}:{params:Promise<{locale:string}>}){
  ...
  const locale=raw as Locale;
  const copy=labels[locale];
  const articles=intlArticles[locale];
  ...
  return <main data-locale={locale} lang={marketContent[locale].htmlLang} className="inner-page">
    ...
    <section className="article-grid shell">{articles.map(article=><a href={`/${locale}/blog/${article.slug}`} className="article-card" key={article.slug}><span>{article.category}</span><h2>{article.title}</h2><p>{article.excerpt}</p><em>{article.date} · {article.readingTime} <ArrowUpRight/></em></a>)}</section>
    <SiteFooter/>
  </main>;
}
