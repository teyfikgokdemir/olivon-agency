  ...
  return (
    <section className="service-essentials shell" aria-label="Hizmet kapsamı ve çalışma modeli">
      ...
      <div className="service-essentials-grid">
        <article><h3>Kimler için uygun?</h3><ul>{audience.map(item => <li key={item}><CheckCircle2 size={16}/>{item}</li>)}</ul></article>
        <article><h3>Ne teslim ediyoruz?</h3><ul>{deliverables.map(item => <li key={item}><CheckCircle2 size={16}/>{item}</li>)}</ul></article>
        <article><h3>Süreç nasıl ilerler?</h3><ul>{process.map(item => <li key={item}><CheckCircle2 size={16}/>{item}</li>)}</ul></article>
      </div>
