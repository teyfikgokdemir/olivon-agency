...
export function FloatingActions() {
  ...
  const whatsappMessage = encodeURIComponent("Merhaba Olivon, web/e-ticaret projem için bilgi almak istiyorum.");
  ...
  return (
    <>
      <a className="whatsapp-float" href={`https://wa.me/905013484838?text=${whatsappMessage}`} target="_blank" rel="noreferrer" aria-label="WhatsApp ile iletişime geç">
      ...
      <button className={`scroll-top ${visible ? "is-visible" : ""}`} type="button" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} aria-label="Sayfanın başına dön">
        <ArrowUp />
      </button>
    </>
  );
